...
- **Палитра** — не набор красок, а шкала из двенадцати ступеней на каждый
  смысловой цвет. Задаёшь четыре краски, остальные тридцать считаются.
  Наведение и нажатие — соседние ступени, а не проценты от цвета кнопки.
...
Самое частое, за чем сюда приходят, лежит в трёх файлах:
...
каждый смысловой цвет (нейтральный, фирменный, красный). Задаются четыре
краски на тему — бумага, чернила, фирменный, красный, — остальные тридцать
считаются. Структура взята у Radix Colors, пороги контраста — из WCAG 2.2.

Проверить свой набор: `npm run check:palette`.
