...
2. **Выбор.** Панель на витрине (`LOOK_PICKER=on`), вход — полоса «Look»
   после страницы, своим местом, кнопка «Open the panel»: поверх страницы
   ничего не лежит (И344); открытая панель сама открывается и на следующей
   странице. Раздел —
