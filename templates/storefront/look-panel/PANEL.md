...
| System | Buttons | Main button | заливка главной `--ctrl-btn-fill-pop`, `-ink-pop`, `-edge-pop`, `-tint-pop`, `-frost-pop`, `-rim-pop` | Fill, Ink, Gradient (градиент ко второму концу краской строителя палитры `--pop-grad` — элементы 06, 19), Glass (стекло марки `--pop-glass` долей, под ним размыто то, что под кнопкой; блик кромки `--pop-rim` — элемент 66) |
...
| System | Fields | Messages | сообщение всей формы `--say-look` — строкой или заметкой тона сигнала со знаком; строка под полем остаётся строкой (И420) | Line (умолчание), Note (элементы 29, 31) |
| System | Links | Under the hand | краска ссылки «куда ведёт» под рукой `--go-hover` — стрелка едет у всех; краска своя или марки для текста (И397) | Plain (умолчание), Brand (элемент 11) |
...
| Admin | Product page | Options | вид выбора варианта `--seg-look` — сегменты примитива `seg` (И396) | Chips (умолчание), Joined (встык — элемент 49), Tray (в подложке — элемент 50) |
| Admin | Card | Sort button | подпись порядка полки `--sort-label` — снаружи кнопки или внутри, со знаком порядка и галкой у выбранного (И395) | Label beside (умолчание), Label inside (элемент 63) |
...
| Admin | Home | Layout | состав главной (`home`): порядок блоков и раскладка каждого; слова, снимки, протокол и способы доставки — данные страницы, одни на все варианты (бриф — `docs/design/home.md` набора) | Scene (умолчание), Shop first, Lab report first, Headline first, Cabinet, Showroom, Poster |
| Admin | Home | Shelf chips | знак полки у начала фишки `--chip-sign`; сам знак — данные полки (`Collection.sign`, имя знака из листа), у полки без знака фишка словом (И422) | Word (умолчание), Sign and word (элемент 65) |
