...
test('look: the site stylesheets are emitted, never a second set; the property list is not behind them', () => {
  ...
  assert.deepEqual([...groups].sort(), ['button', 'card-buy', 'corners', 'face', 'field', 'field-label', 'go-hover', 'head-icons', 'marker', 'pair-look', 'palette', 'pdp-edge', 'pdp-gallery', 'pdp-thumbs', 'say-look', 'scale', 'seg-look', 'shadow', 'shelf-cols', 'shot-frame', 'sort-label', 'tick', 'width'])
