...
test('look values: each kind takes its own shapes', () => {
  ...
  for (const v of ['none', 'var(--sh-raised)']) assert.ok(valid('shadow', v), v)
...
test('look values: nothing that closes a declaration, a block or a tag gets through', () => {
  ...
  assert.ok(!valid('keyword', 'block'), 'слово не из списка рода')
...
test('look values: a bad property falls back alone and is named; the rest of the look stays', () => {
  ...
  assert.equal(acceptValues({ header: 'mega' }, slots, HEADERS).look.header, HEADERS[0])
})
...
test('look values: the style block carries the values and the self-hosted fonts', () => {
  ...
  const css = lookCss({ header: HEADERS[0], vars: { '--a-9': '#112233' }, fonts: [font], names: {} })
  ...
  assert.equal(lookCss({ header: HEADERS[0], vars: {}, fonts: [], names: {} }), '')
})
