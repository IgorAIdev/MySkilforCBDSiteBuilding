...
const FILES = [
  ...
  /* Храповик по переносимости. Едет с первого дня, а не «к переносу»:
     правило заказчика — всё, что производим, должно становиться на разный
     движок и бекенд, а перенести готовое означает переписать его. Проверка
     читает файлы, ей не нужны ни сборка, ни браузер, ни даже второй
     движок в проекте: пока переносимого слоя нет, все шесть семей молчат
     нулём и ждут первого `packages/`. */
  'tools/check-port.mjs',
  /* Одна большая проверка: список берёт из реестра этапов, а не хранит
     второй копией. Заведена по слову заказчика — «запущу в финале одну
     проверку, чтоб она знала, что проверять». */
  'tools/check-all.mjs',
  /* Список семей — общий у проверки и у этой сборки. */
  'tools/code-families.mjs',
  'tools/css-families.mjs',
  'tools/craft-families.mjs',
  'tools/port-families.mjs',
  ...
  /* Реестр проверок с человеческими именами и словами-приметами плюс хук,
     который разбирает по нему фразу заказчика. Вторая половина того же
     слова: «я эти проверки не запомню, они должны срабатывать от моего
     произвольного написания». Едут парой — реестр без хука работает только
     когда о нём вспомнили, хук без реестра не работает вовсе. */
  'tools/checks.mjs',
  'tools/hook-on-prompt.mjs',
  /* Замок самосовершенствования: проверка не живёт без правила, а правка
     спрашивает про правило. Без этих двух файлов набор в новом проекте
     снова начнёт накапливать проверки без причин — то есть ровно то, с
     чего скилл и начался. */
  'tools/check-rules.mjs',
  'tools/hook-on-stop.mjs',
...
writeFileSync(join(OUT, 'tools/rules-baseline.json'),
  JSON.stringify({ drift: 0 }, null, 2) + '\n')
writeFileSync(join(OUT, 'tools/code-baseline.json'),
  JSON.stringify(emptyCodeBaseline(), null, 2) + '\n')
writeFileSync(join(OUT, 'tools/port-baseline.json'),
  JSON.stringify(emptyPortBaseline(), null, 2) + '\n')
...
/* Пары «образец на листе набора — тот же предмет в магазине» — данные ПРОЕКТА,
   ...
   список вместе с объяснением, как его заполнять, — иначе проверка нового
   проекта ищет чужие селекторы и краснеет на пустом месте. */
writeFileSync(join(OUT, 'tools/sheet-samples.mjs'),
  `/**
...
| \`.claude/skills/craft/\` | экран: закон в \`SKILL.md\` (десять запретов, три шкалы, двенадцать примитивов), разбор по темам в \`references/\`, переносимость, проверки |
