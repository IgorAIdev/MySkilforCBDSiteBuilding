 ...
 * как `check:seo` — измеримая половина СЕО-скиллов (`docs/skills.md`):
 * детектор самого impeccable — бинарь, который его запускатель скачивает при
 * первом запуске, — набор не везёт (`.claude/skills/README.md`, «Чего
 * сознательно нет»). Поэтому запреты и рефлексы из его справочников,
