...
export function scale(paper, ink, seed, mode, on = null) {
  ...
  const stepped = (want, cap, i) => {
    let target = cap === null ? want : behind(want, cap)
  ...
  const row = PROFILE[mode].map((want, i) => {
    ...
    if (i < 8) return stepped(want, seed ? solid - dir * 1.5 : null, i)
...
export function roles(set, mode) {
  ...
  out['--ring'] = firstReaching(a, n[0], NEED.control, 7)
  return out
}

/** Набор краской в CSS. Первый набор файла стоит на корне, каждый — ещё и
 *  под своим именем, чтобы витрина могла дать заказчику выбор наборов, не
 *  пересобирая стили. */
...
export function auditPalette(seed, mode) {
  ...
  const jobs = { error: 'ошибки', sale: 'скидки', warn: 'предупреждения', ok: 'наличия' }
  for (const job of STATUS) {
