 ...
 * этапа. Здесь — то, что из impeccable видно по файлу. Его собственный
 * детектор (`impeccable detect`) — бинарь, который запускатель скачивает при
 * первом запуске; набор его не везёт (`.claude/skills/README.md`, «Чего
 * сознательно нет»), и проверка, которая на каждой машине говорила бы
 * «пропущена», была бы правилом без реализации. Поэтому запреты и рефлексы
 * справочников impeccable (`craft-floor.md`, `typeset.md`, `layout.md`)
 * переписаны сюда семьями — как `check:seo` переписал СЕО-скиллы. Семьи,
 * подписи и строки источника — `tools/design-families.mjs`.
 *
